...
public:
    CMatrix();                                            // Init with 0, 1*1
    CMatrix(int r, int c);                                // Init with 0
    CMatrix(int r, int c, T *matrix_arr);                 // Init with array
    CMatrix(int r, int c, std::vector<T> &matrix_arr);    // Init with vector array
...
template <typename T>
CMatrix<T>::CMatrix(int r, int c, std::vector<T> &matrix_arr)
{
    // 这里需不需要检查正整数？
    try
    {
...
CMatrix<T> CMatrix<T>::slice(int r_start, int r_end, int c_start, int c_end)
{
    try
    {
        if (r_end >= len_r || c_end >= len_c || r_start < 0 || c_start < 0)
            throw InvalidException();
...
CMatrix<T> CMatrix<T>::convolution(CMatrix<T> kernal, std::string type)
{
    try
    {
        if ((type == "valid" && (len_r < kernal.len_r || len_c < kernal.len_c)) || (type == "same" && (kernal.len_r % 2 != 1 || kernal.len_c % 2 != 1)))
            throw InvalidException();
...
int CMatrix<T>::eigenvalues_vectors()
{
    ...
    if (len_r == 2)
    {
        ...
        std::cout << "First eigenvector is: " << std::endl
                  << eigenvector_1[0].real() << "+(" << eigenvector_1[0].imag() << "i)" << std::endl;
        ...
        std::cout << "Second eigenvector is: " << std::endl
                  << eigenvector_2[0].real() << "+(" << eigenvector_2[0].imag() << "i)" << std::endl;
    ...
    if (len_r == 3)
    {
        ...
        for (int k = 0; k < 3; k++)
        {
            ...
            std::cout << k << "th eigenvector is: " << std::endl
                      << eigenvector_1[0].real() << "+(" << eigenvector_1[0].imag() << "i)" << std::endl;
            std::cout << eigenvector_1[1].real() << "+(" << eigenvector_1[1].imag() << "i)" << std::endl;
